...
<p>HTTP is a protocol that allows computers to communicate over the web. When you're surfing the web, googling something, or visiting your favorite website, your browser actually does it through HTTP.</p>

<p>As a protocol, HTTP contains different methods to establish a connection with the websites. For example, one of these methods is <code class="java">get</code>. It's used to get the actual representation of the data from the remote server so that you see what you usually see in your web browser. </p>

<p>Now, knowing all that, let's talk a little about how a request is formed on the web.</p>

<p>You can form the HTTP request with Python's module <code class="java">requests</code>. Any HTTP request contains a similar set of data. This includes <em>the method</em>,<strong> </strong><em>the address</em>, and might also require the <em>headers</em>. As the methods and the address were already discussed above, let's deal with the headers. </p>
...
<p>Think of it as a visit card that your program shows to the website before it can enter it. It then introduces itself as a web browser to the website it's trying to get a page from; otherwise, the website might not allow the connection from something other than a browser. </p>
...
<p>Your program already knows how to send a request. What it still doesn’t know is how to handle incoming data to get the translation examples. This is possible with the<strong> </strong><code class="java">BeautifulSoup</code> package.</p>
...
<ul>
	...
	<li><strong>Safari</strong>: Develop ➤ Show Web Inspector. If you can't see the Develop menu, go to Safari ➤ Preferences ➤ Advanced, and check the Show Develop menu in the menu bar checkbox. </li>
...
<p>Here you can see the results that are almost readable, but there are a lot of quotes and commas. They came from the web page you received with requests. The next stage is all about the representation!</p>